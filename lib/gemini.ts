...
// Modelos de geração (free tier). Ordem = tentativa: o 1º que conectar é usado.
const GEN_MODELS = (process.env.GEMINI_GEN_MODELS ?? 'gemini-2.5-flash,gemini-2.5-flash-lite')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
