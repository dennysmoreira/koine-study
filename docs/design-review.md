...
| 8 | Primitivo `<BottomSheet>` compartilhado + focus trap | VIS-5 | P1 | 🟡 Parcial — criado `components/BottomSheet.tsx` (scrim+painel+handle+Escape+**focus trap+retorno de foco**); migrados `TokenSheet`, `HebrewWordSheet`, `CrossRefsSheet`, `AnnotationSheet`. Verificado no preview (após restart: erro de HMR por módulo novo era espúrio). **Follow-up:** migrar `NavSheet`/`VersionsSheet` (internos do Comparator), `StudyModal`, `ReaderHelp` (usa portal), `StudyWorkspace`, picker da `VerseSelectionBar` |
